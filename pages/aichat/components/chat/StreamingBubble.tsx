...
const StreamingBubble: React.FC<Props> = ({ message, onDone }) => {
  ...
  /* ── Finalize ─────────────────────────────────────────────────────────── */
  const finalize = (responseData: any, newChatId: string, params: NonNullable<ChatMessage["streamParams"]>) => {
    if (finalizedRef.current) return;
    finalizedRef.current = true;
    ...
    dispatch(
      finalizeStreamingMessage({
        ...
        type     : hasVisualization ? "visualization" : "text",
        chatId   : newChatId,
        modelName: message.modelName ?? selectedModel,  // ← use what was stored at send time
      })
    );
  ...
  useEffect(() => {
    const params = message.streamParams;
    if (!params || esRef.current) return;

    const token    = localStorage.getItem("access_token");
    // Generate stream_id locally — lives only in this component and Redux
    const streamId = uuidv4();
    streamIdRef.current = streamId;
    dispatch(setActiveStreamId(streamId));

    // streamParams has only { nlQuestion, projectId, chatId } — original shape
    // model_name and stream_id are appended here from local state
    const qs = new URLSearchParams({
      nl_question: params.nlQuestion,
      project_id : String(params.projectId),
      stream_id  : streamId,
      model_name : message.modelName ?? selectedModel,  // ← correct model
      ...(params.chatId ? { chat_id: params.chatId } : {}),
      token      : token ?? "",
    });
    ...
    es.onmessage = (event) => {
      ...
      switch (data.type) {
        ...
        case "token": {
          ...
          if (!chunk) break;
          // Append entire token at once — no char queue, renders immediately
        ...
        case "done": {
          ...
          const responseData = data.full_response?.data ?? data.full_response ?? {};
          const newChatId    = String(responseData.chat_id ?? params.chatId ?? "");
          finalize(responseData, newChatId, params);
          break;
        }
  ...
  /* ── Abort via window event (fired by ChatInput) ─────────────────────── */
  useEffect(() => {
    const handler = (e: Event) => {
      const { streamId } = (e as CustomEvent).detail ?? {};
      // Accept event if streamId matches OR if no streamId provided (broadcast)
      ...
      setStatusText("");
      dispatch(setActiveStreamId(null));
      // displayedText stays frozen — no finalize dispatch, no Redux update
    };
