...
//   // For bug data, hide the url column (we'll make title clickable instead)
//   const columns = isBugData 
//     ? allColumns.filter(col => col !== 'url')
//     : allColumns;
...
export default function ChatResponseCard({ data }: Props) {
  ...
  // visualization_type "none" means: show text + raw table, no chart
  // For bugs, always treat as "none" to show table
  // For NFR content, treat as "none" but DON'T show raw table (only markdown)
  ...
  return (
    <div className="w-full max-w-full overflow-hidden">
      ...
      {/* Answer / summary text 
          - Suppress markdown tables for bugs (show structured bug table instead)
          - Suppress markdown tables for query results with data (show structured table instead)
          - Allow markdown tables only for NFR content or when no structured data
      */}
      {hasSummary && (
        <MarkdownBlock
